...
//      Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        Eigen::MatrixXd D_new{3, 3};
        D_new.row(0) << svdf.singularValues()[0], 0, 0;
        D_new.row(1) << 0, svdf.singularValues()[1], 0;
        D_new.row(2) << 0, 0, 0;

        F = svdf.matrixU() * D_new * svdf.matrixV().transpose();
    ...
    {
        cv::Vec2d centre = {0.0, 0.0};
        for (auto& v : m) {
            centre += v;
        }
        centre /= static_cast<double>(m.size());
        ...
        double mult = std::sqrt(2) / rms;

        return {
            mult, 0, -centre[0] * mult,
            0, mult, -centre[1] * mult,
            0, 0, 1
        };
    }
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        double inlier_ratio = 0.30;
        double actual_p = std::pow(inlier_ratio, n_samples);
        int n_trials = std::ceil(std::log(1 - desired_p) / std::log(1 - actual_p));
